 ...
 *
 * IMPORTANT: This module sets process.env.PORT, process.env.DATABASE_URL,
 * and process.env.WEB3_ANVIL_RPC at load time to ensure serverConfig
  ...
  if (existingUrl) {
    try {
      ...
      return {
        ...
        port: parseInt(url.port, 10) || 55433,
      }
    } catch {
      // Invalid URL, fall through to defaults
    }
  }
  return { host: "localhost", port: 55433 }
}

const { host: PG_HOST, port: PG_PORT } = getDatabaseConfig()

const TEST_BLOCKCHAIN_PORT_BASE = 58000
...
/**
 * Get the unique blockchain port for this test file
 */
export function getTestBlockchainPort(): number {
  return TEST_BLOCKCHAIN_PORT_BASE + getTestFileIndex()
}

/**
 * Get the Anvil RPC URL for this test file
 */
export function getTestAnvilRpcUrl(): string {
  return `http://127.0.0.1:${getTestBlockchainPort()}`
}
...
const anvilRpcUrl = getTestAnvilRpcUrl()
...
process.env.WEB3_ANVIL_RPC = anvilRpcUrl
...
export async function getTestConfig() {
  ...
  return {
    ...
    BLOCKCHAIN_PORT: getTestBlockchainPort(),
    ANVIL_RPC_URL: getTestAnvilRpcUrl(),
  }
}
